...
export default function Home() {
  ...
  const handleSaveChatPdf = useCallback(() => {
    if (typeof window === "undefined") return;
    toast.info("Opening print dialog to save chat as PDF…");
    window.print();
  }, []);

  const handleSyncSupabase = useCallback(() => {
    toast.promise(
      (async () => {
        const success = await reloadThread();
        if (!success) {
          throw new Error("sync-failed");
        }
      })(),
      {
        loading: "Syncing chat with Supabase…",
        success: "Chat synced with Supabase",
        error: "Unable to sync chat with Supabase",
      }
    );
  }, [reloadThread]);

  const handleShowMemoryTree = useCallback(() => {
    setMemoryTreeVisible(true);
  }, [setMemoryTreeVisible]);

  const handleToggleSuperpowers = useCallback(() => {
    setSuperpowersModalVisible((prev) => !prev);
  }, [setSuperpowersModalVisible]);

  const handleGlowMode = useCallback(() => {
    toggleTheme();
  }, [toggleTheme]);

  useElectronMenu({
    "toggle-orb": handleToggleOrb,
    "save-chat-pdf": handleSaveChatPdf,
    "sync-supabase": handleSyncSupabase,
    "show-memory-tree": handleShowMemoryTree,
    "toggle-superpowers": handleToggleSuperpowers,
    "glow-mode": handleGlowMode,
  });
  ...
  return (
    ...
    >
      ...
      <div className="flex-shrink-0  mt-16">
        <Header
          theme={theme}
          personaName={getCurrentPersona()?.name || ""}
          personaDescription={getCurrentPersona()?.description || ""}
      ...
      {showTokenUsage && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          ...
          >
            ...
            <TokenUsageComponent userId={user?.id || ""} theme={theme} />
          </motion.div>
        </div>
      )}
