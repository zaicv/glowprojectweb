...
export function NavThreads() {
  ...
  return (
    <SidebarGroup className="space-y-4 px-2">
      <div className="flex items-center justify-between rounded-2xl bg-white/5 px-3 py-2 backdrop-blur">
        <SidebarGroupLabel className="text-xs font-semibold uppercase tracking-[0.3em] text-white/70">
          Chats
        </SidebarGroupLabel>
        <button
          type="button"
          onClick={handleCreateThread}
          className="inline-flex items-center gap-1 rounded-xl bg-white/15 px-3 py-1 text-xs font-medium text-white transition hover:bg-white/25"
        >
      ...
      <div className="px-1">
        <div className="relative">
          <Search className="pointer-events-none absolute left-3 top-1/2 h-3 w-3 -translate-y-1/2 text-white/40" />
          <SidebarInput
            value={search}
            onChange={(e) => setSearch(e.target.value)}
            placeholder="Search conversations"
            className="h-9 rounded-2xl border-white/10 bg-white/5 pl-8 text-sm text-white placeholder:text-white/50"
          />
        </div>
      </div>
      ...
      <div className="max-h-[50vh] space-y-4 overflow-y-auto pr-1 pb-2">
        ...
        {!loading && totalThreads === 0 && (
          <div className="rounded-2xl border border-dashed border-white/10 px-4 py-6 text-center text-xs text-white/60">
          ...
          Object.entries(grouped).map(([label, items]) =>
            items.length ? (
              <div key={label} className="space-y-2">
                <p className="px-2 text-[11px] font-medium uppercase tracking-[0.3em] text-white/40">
                  {label}
                </p>
                  ...
                  {items.map((thread) => {
                      ...
                      return (
                        <div key={thread.id} className="px-2">
                          <SidebarInput
                            ...
                            className="h-8 rounded-xl bg-white/10 text-sm"
                            autoFocus
                          />
                        </div>
                      );
                    ...
                    return (
                      ...
                      >
                        ...
                        >
                          <span className="flex h-7 w-7 items-center justify-center rounded-xl bg-white/15 text-xs text-white">
                        ...
                        <DropdownMenu>
                          <DropdownMenuTrigger asChild>
                            <button
                              type="button"
                            className="absolute right-2 top-1/2 -translate-y-1/2 rounded-full p-1 text-white/50 opacity-0 transition hover:bg-white/10 hover:text-white group-hover:opacity-100"
                          >
                          ...
                          <DropdownMenuContent
                            side="right"
                            align="start"
                            className="w-44 rounded-2xl border border-white/10 bg-zinc-900/95 text-white backdrop-blur-xl"
                          >
                            <DropdownMenuItem
